...
![HUWARI 메인 화면 — 코디 캔버스 + 점수·피드백 패널](docs/screenshots/01-hero-home.png)
...
<!-- 📸 SCREENSHOT: History 페이지(스타일 리포트 카드 + 코디 카드 그리드) -->
![History 페이지 — 내 스타일 리포트 + 코디 카드 12장](docs/screenshots/02-history-overview.png)

**Info**는 서비스 설명과 사용 가이드를 모아 둔 페이지다.
...
> **참고(현재 저장소)**: 위 다이어그램·목록은 **예전 방식** 기준이다. 지금 `main.py`의 조화 API는 **`POST /api/predict-harmony`** 만이며(FashionHarmony + FashionCLIP), **`/api/harmony-score`는 제공하지 않는다.**
...
| **CLIP 하이브리드 멀티모달** (Kalashi & Teimourpour, 2024) | CLIP 기반 고성능 경향 | **CLIP의 패션 이해 능력**을 확인. 다만 조화 판단의 메인은 **FashionHarmony(Set Transformer)** 로 두고, **CLIP은 색 조화 보조(총점 25%)·말풍선 문구 매칭**에만 활용 |
...
사람은 옷을 하나씩 따로 평가하지 않는다. 상의·하의·신발·아우터가 서로 어떻게 어울리는지 같이 보고 코디의 조화를 판단한다. FashionHarmony는 이 과정을 모델로 옮기기 위해 설계한 통합 조화 모델이다. 한 코디 안의 아이템들을 **하나의 세트(Set)** 로 묶어 백본·속성 헤드·Set Transformer에 같이 흘려보내, 개별 분류와 전체 조화 점수를 같은 표현 공간에서 만들어 낸다.
...
Set Transformer는 self-attention으로 **상의–하의, 상의–신발, 하의–신발** 같은 쌍별 관계를 동시에 학습한다. 그래서 코디 전체의 조화를 따로 후처리하지 않고 한 번에 모델링할 수 있고, 학습된 attention 가중치가 그대로 XAI 설명에도 쓰인다.
...
| 평가 | 세트 호환성 | **AUC 약 0.912** (SigLIP 라벨·해당 실험 기준) |
...
1. **스타일은 사람마다 기준이 다르다.** 「캐주얼」「스트리트」 같은 단어는 사람·상황마다 받아들이는 폭이 달라서, 같은 옷도 누구는 캐주얼로, 누구는 스트리트로 본다. 그래서 **하나의 정답 라벨로 묶기 어렵다.** 실제로 스타일 분류는 라벨러끼리도 의견이 갈리는(inter-annotator agreement가 낮은) 태스크로 알려져 있다.
...
4. **수정값은 서버에 실제로 반영된다.**
   ...
   - **조화 점수**: **FashionHarmony(이미지) 75% + FashionCLIP 색 25%** 자체는 이미지 기반이지만, 캔버스에 메인 아이템이 있으면 `harmony.py` **룰북 총점과 50% 병합**한다. 룰북·XAI 둘 다 사용자가 고친 **재질·패턴·스타일·색**을 읽으므로, 수정에 따라 **`score_total`·`reasons`** 가 같이 움직인다.
...
flowchart TB
  ...
  subgraph analyze ["분석 (서버)"]
    ...
    HARMONY["조화 점수<br/>FashionHarmony 75% +<br/>FashionCLIP 색 25%"]
...
1. 여러 아이템을 한 코디로 보고 **조화 점수(0~100)** 를 낸다. FashionHarmony가 만든 세트 점수에 FashionCLIP의 색 점수를 **75% / 25%** 로 합쳐 최종 점수를 만든다.
...
- **색 분석** — 아이템별 픽셀을 묶어 대표 색을 뽑고, UI 색칩과 룰북 색 점수에 함께 사용한다. 총점의 색 25%는 따로 FashionCLIP이 코디 이미지를 보고 계산한다.
...
<!-- 📸 SCREENSHOT: 코디 업로드 + 분석 결과 패널 전체 -->
![Home 업로드 모드 — 캔버스 배치 + 색·속성 분석 + 점수 패널](docs/screenshots/03-home-upload.png)
...
- **점수 합성** — 메인 의류에 대한 FashionHarmony 세트 점수와, 코디 전체에 대한 FashionCLIP 색 점수를 **75% / 25%** 로 합쳐 0~100으로 환산한다.
- **75% / 25% 비율을 둔 이유** — FashionHarmony를 조화 판단의 주 모델로 두고, FashionCLIP 색 점수는 보조 신호로만 반영하기 위해서다. 패션 조화는 색뿐 아니라 아이템 간 관계, 스타일, 재질, 패턴이 함께 작용하므로 색 점수가 총점을 과하게 지배하지 않도록 25%로 제한했다. 다만 이 비율은 사용자 평가나 추가 실험으로 최적화한 값은 아니며, 서비스 구현 단계에서 정한 초기 가중치다.
...
<!-- 📸 SCREENSHOT: 저장 → 카드 추가 흐름을 실제 화면 2~3컷 모음으로 보여주면 좋음 -->
![저장 → History 카드 추가 흐름(Home 저장 → History 신규 카드)](docs/screenshots/08-save-to-history.png)
...
| 동작 | 화면에서 보이는 것 |
|------|--------------------|
| **실시간 분석** | 카메라를 켜면 일정 주기로 프레임을 분석해, 영상 위에 옷 영역 박스를 그리고 조화 점수·피드백을 갱신한다. 캔버스에는 아이템이 추가되지 않는다. |
...
3. 관절이 안 잡히면 **YOLOv8** 사람 영역으로 비율을 추정해 옷을 잘라 낸다. 옷이 전혀 잡히지 않으면 안내 문구만 돌려준다.
4. 잘라낸 의류 이미지를 [§5.3](#predict-harmony-api)와 같은 방식으로 FashionHarmony·FashionCLIP에 넣어 0~100점을 만든다.
5. 크롭별 색·속성을 모아 분석 패널에 채우고, 피드백 문장은 [XAI](#xai-explainability) 절의 규칙대로 만든다.
...
- **색 점수**: 코디 합성 이미지와 "조화로운 색 / 충돌하는 색" 문장의 유사도를 비교해 0~100점을 만든다. 총점에 **25%** 로 반영된다.
...
- **60점 이상**: Attention 1줄 + 색상 1~2줄 + 재질·패턴·스타일 각 0~1줄 + 종합 요약 1줄 (긍정·중립 톤)
...
<!-- 📸 SCREENSHOT: 개인화 한 줄이 Gini 말풍선 첫 줄에 끼워진 화면 -->
![개인화 한 줄 — Gini 첫 말풍선에 prepend된 모습](docs/screenshots/06-personalization-line.png)

<!-- 📸 SCREENSHOT: History 페이지 「내 스타일 리포트」 카드 클로즈업 -->
![내 스타일 리포트 카드 — top 스타일·색·평균 점수](docs/screenshots/07-style-report-card.png)
