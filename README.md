...
패션 아이템 조합의 조화도를 분석하는 웹 애플리케이션입니다.  

## 웹사이트 소개

HUWARI는 "지금 가진 옷 조합이 잘 어울리는지"를 빠르게 확인할 수 있도록 만든 코디 분석 웹사이트입니다.
사용자는 이미지 업로드와 간단한 배치만으로 조화 점수와 속성 분석 결과를 받아볼 수 있으며, 결과를 히스토리에 저장해 이전 코디와 비교할 수 있습니다.
...
- `Home`
  - 메인 작업 공간입니다.
  - before/after 아이템을 올리고 분석을 실행하면 점수와 해석 결과를 확인할 수 있습니다.
- `History`
  - 저장된 분석 결과를 시간순으로 조회합니다.
  - 과거 코디를 다시 불러와 현재 조합과 비교할 수 있습니다.
- `Info`
  - 서비스 설명 및 사용 가이드를 제공합니다.

### 사용자 흐름

1. 이미지를 업로드하고 필요 시 배경 제거를 적용합니다.
2. 아이템의 색상/속성 정보를 추출합니다.
3. 조화 점수(총점 + 세부 항목)를 계산하고 결과를 해석합니다.
4. 마음에 드는 결과는 히스토리에 저장해 재사용합니다.
...
- **이미지 업로드 및 전처리**
  - 단일 또는 다중 의류 이미지를 업로드하고 분석 파이프라인에 입력합니다.
  - 업로드 이미지는 서버에서 포맷을 정규화하여 후속 모델 추론에 사용합니다.

- **배경 제거(RemBG)**
  - 의류 객체를 중심으로 배경을 제거해 색상/속성 분석의 노이즈를 줄입니다.
  - 결과는 PNG 형태로 반환되어 전/후 비교나 레이아웃 합성에 활용할 수 있습니다.

- **대표 색상 추출 및 시각화**
  - 각 아이템에서 주요 색상 팔레트를 추출하고 비율(percentage) 정보를 함께 제공합니다.
  - 추출된 색상은 조화 점수 계산과 UI 상의 색상 요약 카드에 활용됩니다.

- **조화 점수 예측(핵심)**
  - before(기준 코디)와 after(추천/후보 아이템) 관계를 바탕으로 조화 점수를 계산합니다.
  - 출력 항목:
    - 총점(`score_total`)
    - 세부 점수(`score_color`, `score_texture`, `score_pattern`, `score_style`)
    - 해석 문장(`reasons`) 및 디버그 정보(`debug`)
  - 상황에 따라 이미지 기반 ranker와 규칙 기반 스코어링 로직이 폴백 구조로 동작합니다.

- **패션 속성 분류(MTL)**
  - Multi-Task Learning 모델로 아래 3개 태스크를 동시에 예측합니다.
    - 재질(Material / Texture)
    - 패턴(Pattern)
    - 스타일(Style)
  - 학습용 세부 클래스를 사용자 표시용 카테고리로 매핑해 직관적으로 제공합니다.

- **의류 타입 분류**
  - 상의/하의/모자/신발/악세서리 카테고리를 자동 분류합니다.
  - CLIP 기반 분류를 우선 사용하고, 실패 시 ImageNet 계열 분류 모델로 자동 폴백합니다.

- **사람 영역 감지 기반 보조 신호**
  - YOLO를 활용해 인물 포함 여부 및 박스 기반 비율 정보를 추출합니다.
  - 분류 결과 보정 또는 후처리 조건 판단에 보조 신호로 사용됩니다.

- **히스토리 저장/조회/삭제**
  - 분석 결과(아이템 배치, 조화 점수, 생성 시각)를 히스토리로 저장합니다.
  - 히스토리 페이지에서 정렬/필터 상태를 유지한 채 결과를 다시 불러올 수 있습니다.

- **사용자 경험(UX) 보강**
  - 페이지 이동 시 현재 작업 상태를 최대한 복원하도록 로컬/세션 스토리지를 활용합니다.
  - 인터랙션 요소(버튼 상태, 안내 말풍선, 커서 트레일 등)를 통해 분석 흐름을 직관적으로 제공합니다.
...
- `models/harmony_ranker.py`
  - `MHAttentionRanker`를 사용해 before 세트와 after 아이템의 관계를 점수화합니다.
  - 입력 임베딩은 `EfficientNet-B0`(timm) 기반으로 추출합니다.
- `models/fashion_mtl.py`
  - Multi-Task Learning으로 3개 태스크를 동시에 예측합니다.
  - 태스크: 재질(Material), 패턴(Pattern), 스타일(Style)
- `main.py`
  - 의류 타입 분류 시 CLIP을 우선 시도하고 실패 시 ImageNet 기반 분류기로 폴백합니다.
...
- 후보군이 커질수록 난이도가 올라가며 두 모델 모두 지표가 하락합니다.
- 모든 후보군 설정에서 `MH-Attn`이 `MLP`보다 우세합니다.
- 특히 `candidates=50`에서 격차가 가장 큽니다.
